...
class PrefHelper {
  ...
  Future<bool> saveUID(String uid) async {
    return await _pref.setString('uid', uid);
  }

  String get uid => _pref.getString('uid') ?? '';

  Future<bool> removeuid() async {
    return await _pref.remove('uid');
  }
  ...
  Future<bool> setFavorite(List<String> code) async {
    return await _pref.setStringList('favorite', code);
  }

  List<String> get favorite => _pref.getStringList('favorite') ?? [];
}
